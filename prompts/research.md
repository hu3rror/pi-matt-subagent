...
2. Call the `research` tool once with `task: $@` and that findings path. Match the budget tier to the task scale: keep the default `standard` unless the question is a narrow fact-check — `tight` is only for quick single-point verifications, and a doc-reading task on `tight` gets cut by the wall-clock cap. The tool returns immediately with a handle — do NOT wait, and do NOT poll for the file.
